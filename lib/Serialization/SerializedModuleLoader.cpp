...
serialization::Status SerializedModuleLoaderBase::loadAST(
  ...
  serialization::Status status = loadedModuleFile->associateWithFileContext(
  ...
  if (status == serialization::Status::Valid ||
      (Ctx.LangOpts.AllowModuleWithCompilerErrors &&
       (status == serialization::Status::TargetTooNew ||
        status == serialization::Status::TargetIncompatible))) {
    Ctx.bumpGeneration();
